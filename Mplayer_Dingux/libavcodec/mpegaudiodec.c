...
#ifdef JZ47_OPT
	    table_4_3_value[i] = decode_table_4_3_value[i];
            table_4_3_exp[i] = decode_table_4_3_exp[i];
#else
            double f, fm;
            int e, m;
...
//            av_log(NULL, AV_LOG_DEBUG, "%d %d %f\n", i, m, pow((double)i, 4.0 / 3.0));
        ...
        for(i=0; i<512*16; i++){
            int exponent= (i>>4);
#ifdef JZ47_OPT
            expval_table[exponent][i&15] = decode_expval_table[exponent][i&15];
            if((i&15)==1)
                exp_table[exponent]= decode_exp_table[exponent];
