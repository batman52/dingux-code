...
#define COEFF(c,_t,_l,s,e) 				\
      __extension__({ \
         ...
         signed long lo;\
         int      shift = e - 5;\
         __asm__ __volatile(" mult %0, %1" \
                            ::"%r"(l), "r"(t)\
                           );\
         __asm__ __volatile("mfhi %0" \
                            :"=r"(hi));\
         __asm__ __volatile("mflo %0"  \
                            :"=r"(lo));\
         __asm__ __volatile("sll %0,%1,16"\
                           :"=r"(hi)\
                           :"d"(hi));\
         __asm__ __volatile("srl %0,%1,16"\
                           :"=r"(lo)\
                           :"d"(lo));\
         __asm__ __volatile("or  %0,%1,%2"\
                           :"=r"(hi)\
                           :"d"(hi),"d"(lo));\
          if(shift >= 0)\
             __asm__ __volatile("sra %0,%1,%2"\
                           :"=r"(c)\
                           :"d"(hi), "d"(shift));\
          else\
             __asm__ __volatile("sll %0, %1, %2"\
                           :"=r"(c)\
                           :"d"(hi), "d"(-shift));\
      })
#endif
		       ...
		       level_t level, int dither, int end)
{
	...
	case 0:
		...
		//COEFF (coeff[i], dither_gen (state), level, factor, exp[i]);
                value = dither_gen(state);
		COEFF (coeff[i], value, level, factor, exp[i]);
#ifndef LIBA52_FIXED
                coeff[i] *= LEVEL_3DB;
#else
                //coeff[i] = ((long long)coeff[i] * FIX_LEVEL_3DB)>>31; 
                coeff[i] = MUL(coeff[i],SAMPLE(LEVEL_3DB)); 
#endif
		continue;
	    } else {
		coeff[i] = 0;
			...
			if (dithflag[ch]){
			    ...
			    COEFF (samples[ch][i], dither_gen (state),
				   cplco[ch], scale_factor, exp[i]);
                            //samples[ch][i] =
                              //   ((long long)samples[ch][i] * FIX_LEVEL_3DB)>>31;
                            samples[ch][i] =MUL(samples[ch][i], SAMPLE(LEVEL_3DB));
#endif
