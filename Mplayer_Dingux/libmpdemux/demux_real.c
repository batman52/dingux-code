...
//		F("aaaa %d\n",priv->current_vpacket);
		if((priv->current_vpacket - 1) > 0)
		{
 
			int pos;
			priv->current_vpacket--;
			pos = priv->index_table[demuxer->video->id][priv->current_vpacket].offset; 
//			F("aaaa pos = %d\n",pos);
...
//		F("bbbb %d\n",priv->current_vpacket);
		if((priv->current_vpacket - 1) > 0)
		{
				...
				int pos;
				pos = priv->index_table[stream_id][priv->current_vpacket - 1].offset; 
				F("bbbb pos = %d\n",pos);
				stream_seek(demuxer->stream, pos);	
